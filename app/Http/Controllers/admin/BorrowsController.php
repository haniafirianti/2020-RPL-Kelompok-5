...
class BorrowsController extends Controller
{
    ...
    public function add_borrow()
    {
      ...
      $borrow  = Borrows::all();
      $count   = 0;
      return view('admin/borrows/add-borrow', ['borrow' => $borrow, 'books' => $book, 'students' => $student, 'count' => $count]);
    }
    ...
    {
    $borrow = Borrows::findOrFail($borrow_id);
    $book_id = $borrow->borrow_book_id;
    $borrow->borrow_back_date = Carbon::now();
    $borrow->save();

    $book = Books::whereBookId($book_id)->first();
    $jml_book = $book->book_total;
    $jml = $borrow->borrow_total_books += $jml_book;
    $book->book_total = $jml;
    $book->save();
